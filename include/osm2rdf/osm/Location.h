#ifndef OSM2RDF_OSM_LOCATION_HPP
#define OSM2RDF_OSM_LOCATION_HPP
...
namespace osm2rdf::osm {
    ...
    class Location {
        ...
        static constexpr int32_t Y_SHIFT = 900000000;
    ...
    public:
        ...
        /**
         * Create Location from osmium::Location
         */
        ...
        constexpr Location(const int64_t x, const int64_t y) noexcept :
            ...
            m_y(static_cast<int32_t>(y) + Y_SHIFT) {
        }
        ...
        Location& set_y(const int32_t y) noexcept {
            m_y = (m_y & TAG_BIT) | ((y + Y_SHIFT) & ~TAG_BIT);
            return *this;
        }
        ...
        Location& set_lat(double lat) noexcept {
            m_y = (m_y & TAG_BIT) | ((double_to_fix(lat) + Y_SHIFT) & ~TAG_BIT);
            return *this;
        }
    ...
    inline std::basic_ostream<TChar, TTraits>& operator<<(std::basic_ostream<TChar, TTraits>& out, const osm2rdf::osm::Location& location) {
        if (location) {
            out << '(';
    ...
    namespace detail {
        ...
        template <int N>
        inline size_t hash(const osm2rdf::osm::Location& location) noexcept {
            return static_cast<uint32_t>(location.x()) ^ static_cast<uint32_t>(location.y());
        }

        template <>
        inline size_t hash<8>(const osm2rdf::osm::Location& location) noexcept {
...
#endif
    template <>
    struct hash<osm2rdf::osm::Location> {
        using argument_type = osm2rdf::osm::Location;
        using result_type = size_t;
        size_t operator()(const osm2rdf::osm::Location& location) const noexcept {
            return osm2rdf::osm::detail::hash<sizeof(size_t)>(location);
        }
    };
#ifdef __clang__
...
#endif // OSM2RDF_OSM_LOCATION_HPP
