#ifndef OSM2RDF_OSM_HANDLER_NODE_LOCATIONS_FOR_WAYS_HPP
#define OSM2RDF_OSM_HANDLER_NODE_LOCATIONS_FOR_WAYS_HPP
...
namespace osm2rdf::osm {
    ...
    namespace handler {
        ...
        using dummy_type = osmium::index::map::Dummy<osmium::unsigned_object_id_type, osm2rdf::osm::Location>;
        ...
        public:
            ...
            void node(const osmium::Node& node) {
                ...
                osm2rdf::osm::Location loc = node.location();
                loc.set_tagged(!node.tags().empty());

                const auto id = node.id();
                if (id >= 0) {
                    m_storage_pos.set(static_cast<osmium::unsigned_object_id_type>( id), loc);
                } else {
                    m_storage_neg.set(static_cast<osmium::unsigned_object_id_type>(-id), loc);
                }
            }

            /**
             * Get if node is tagged
             */
            bool get_node_is_tagged(const osmium::object_id_type id) const {
                if (id >= 0) {
                    return m_storage_pos.get_noexcept(static_cast<osmium::unsigned_object_id_type>(id)).is_tagged();
                }
                return m_storage_neg.get_noexcept(static_cast<osmium::unsigned_object_id_type>(-id)).is_tagged();
            }

            /**
             * Get location of node with given id.
             */
            osm2rdf::osm::Location get_node_location(const osmium::object_id_type id) const {
                if (id >= 0) {
...
#endif // OSM2RDF_OSM_HANDLER_NODE_LOCATIONS_FOR_WAYS_HPP
